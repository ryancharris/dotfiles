...
2. Search separately for pages created by the user in the date range and pages last edited by the user in the date range. Request the connector maximum `page_size: 50`; because this search exposes no result cursor, label Notion activity potentially incomplete when a query returns the full cap.
...
First canonicalize candidates into underlying situations using incident links, repository/PR numbers, tickets, services, and confirmed aliases. One situation gets one output item even when several sources describe it. Then rank situations, after adding the active incident.io set described in `references/incidents.md`:

1. Active incident with confirmed customer impact or immediate urgency.
2. Other customer-impact signal or incident requiring action from the user.
3. Other direct action required from the user.
4. Blocker, security, launch, or deadline risk.
...
Suppress bots, mirrors, self-authored repeats already covered by `Yesterday`, generic keyword collisions, casual chatter, and unsupported speculation. Do not suppress a current incident merely because it lacks a watch-term match. Keep at most three situations; render `No new watched signals` rather than adding filler when no candidates qualify. For each item, show a signal label (`Action`, `Risk`, `Decision`, `Watch`, or `Quiet`), the matched incident or topic, a one- or two-sentence summary, and a source link. State source coverage and any truncation outside the three signal slots.
