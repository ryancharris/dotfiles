...
Hydrate each candidate URL separately, with roughly 6–8 calls in flight, so nested fields cannot multiply across a large result limit and exceed GitHub's GraphQL node ceiling:
...
- `Across the org` surfaces at most three new, important situations. Canonicalize related incidents, PRs, Slack threads, and Notion pages into one item per underlying situation; do not repeat the same situation merely because it has customer impact, user involvement, and Transport involvement. Include one visually tagged, clustered `Transport` summary when there is verified, material Transport involvement, and explain the team's involvement, customer or operational impact, and current action or status. Place it at the highest rank justified by its impact; do not let a low-impact Transport item displace a higher-priority incident or customer-impact signal. Rank confirmed customer impact or immediate incident urgency first, material Transport involvement second, and other direct action third; within a tier prefer higher severity and fresher evidence. State why each item matters and which incident or topic matched.
