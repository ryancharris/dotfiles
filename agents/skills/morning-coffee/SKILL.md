...
Honor an explicit format request. Otherwise classify the client context supplied to the agent by surface, not vendor: choose desktop Markdown for a GUI surface — a desktop app, web app, or IDE extension, for either Claude (Claude Desktop, Claude.ai, an IDE extension) or ChatGPT/Codex (ChatGPT desktop or web, Codex Desktop) — and CLI TUI for a terminal surface — Claude Code or the Codex CLI, including when the agent identifies itself only by that name (e.g. "Claude Code") without naming a specific host surface. If the client is unclear, use desktop Markdown. Do not infer the visible client from shell environment variables such as `TERM`, TTY state, or `NO_COLOR`.
